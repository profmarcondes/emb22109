...
sc::time sc::minValue(sc::time a, sc::time b) {
	return (a < b) ? a : b;
}
sc::time sc::maxValue(sc::time a, sc::time b) {
	return (a > b) ? a : b;
}
...
void TimerService::proceed(sc::time elapsed_ms) {
	...
	sc::time time_to_proceed = sc::minValue(time_till_next_task(), elapsed_ms);
	...
	while (time_to_proceed > 0) {
		...
		time_to_proceed = sc::minValue(time_till_next_task(), elapsed_ms - proceeded_time);
	}
}
